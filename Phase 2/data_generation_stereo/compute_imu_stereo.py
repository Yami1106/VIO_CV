"""
compute_imu_stereo.py — Compute IMU data from poses
-----------------------------------------------------
Same as compute_imu.py but reads/writes to /mnt/data/datasets.

Usage:
    python compute_imu_stereo.py --scene_name scene_001_circle
"""
    ...
    with open(csv_path, "r", newline="") as f:
        ...
        for row in reader:
            poses.append({
                ...
                "position": np.array([float(row["px"]), float(row["py"]), float(row["pz"])], dtype=float),
                "quaternion": np.array([float(row["qx"]), float(row["qy"]), float(row["qz"]), float(row["qw"])], dtype=float)
            })
    return poses
...
def compute_body_angular_velocity(quaternions, dt):
    ...
    for i in range(n - 1):
        ...
        omega = np.array([skew_sym[2, 1], skew_sym[0, 2], skew_sym[1, 0]], dtype=float)
        omegas[i] = omega
    omegas[-1] = omegas[-2]
    return omegas
...
def compute_imu_from_poses(poses):
    ...
    for i in range(len(poses)):
        R_bw = quat_to_rotmat(quaternions[i])
        R_wb = R_bw.T
        ...
        imu_rows.append([timestamps[i], gyro[0], gyro[1], gyro[2], accel[0], accel[1], accel[2]])
