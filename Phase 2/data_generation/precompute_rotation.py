"""
precompute_rotation.py
----------------------
Usage:
    python precompute_rotation.py                    # all scenes
    python precompute_rotation.py --scene scene_001_circle  # one scene
"""
...
def integrate_gyro_window(imu_rows, start_idx, end_idx, dt):
    """
    Integrate gyroscope angular velocity over a window to get relative rotation.
    Uses Rodrigues' formula: R_delta = I + sin(θ)*K + (1-cos(θ))*K²
    where θ = |ω|*dt and K is the skew-symmetric matrix of the rotation axis.

    Returns: relative rotation quaternion [qx, qy, qz, qw]
    """
    R_accum = np.eye(3)
...
def process_scene(scene_name):
    """Add precomputed gyro rotation to samples.csv"""
    ...
    # Determine dt from IMU timestamps
    if len(imu_rows) > 1:
    ...
    # Compute gyro rotation for each sample
    ...
    # Write updated samples.csv with new columns
    new_fieldnames = list(fieldnames) + ["gyro_qx", "gyro_qy", "gyro_qz", "gyro_qw"]
    ...
    # Verify: compare gyro rotation with GT rotation from poses
    ...
    print(f"  {scene_name}: {len(samples)} samples, gyro vs GT rotation error: {mean_err:.6f}°")
...
def main():
    ...
    print("\nDone. Now re-run split_dataset.py to include gyro_q* columns in train/val/test CSVs.")
