...
class MSCKF(object):
    ...
    def initialize_gravity_and_bias(self):
        ...
        gyro_sum = np.zeros(3)
        acc_sum = np.zeros(3)
        for imu_msg in self.imu_msg_buffer[:200]:
        ...
        imu_state.gyro_bias = gyro_sum / 200.0

        # Find the gravity in the IMU frame.
        gravity_imu = acc_sum / 200.0
        
        # Normalize the gravity and save to IMUState          
        gravity_imu = gravity_imu / np.linalg.norm(gravity_imu)

        # Initialize the initial orientation, so that the estimation
        # is consistent with the inertial frame.
        gravity_world = IMUState.gravity / np.linalg.norm(IMUState.gravity)
        imu_state.orientation = from_two_vectors(gravity_world, -gravity_imu)
        imu_state.orientation_null = imu_state.orientation.copy()
        imu_state.acc_bias = np.zeros(3)
    ...
    def process_model(self, time, m_gyro, m_acc):
        ...
        Fdt = F * dt
        Phi = np.identity(21) + Fdt + 0.5 * (Fdt @ Fdt) + (1.0 / 6.0) * (Fdt @ Fdt @ Fdt)
        ...
        u = to_rotation(imu_state.orientation_null) @ IMUState.gravity
        u = u / np.linalg.norm(u)

        A1 = Phi[0:3, 0:3]
        Phi[0:3, 0:3] = A1 - np.outer(A1 @ u, u) + np.outer(u, u)

        A2 = Phi[6:9, 0:3]
        w1 = skew(imu_state.velocity_null - imu_state.velocity) @ IMUState.gravity
        Phi[6:9, 0:3] = A2 - np.outer(A2 @ u, u) + np.outer(w1, u)

        A3 = Phi[12:15, 0:3]
        w2 = skew(dt * imu_state.velocity_null + imu_state.position_null - imu_state.position) @ IMUState.gravity
        Phi[12:15, 0:3] = A3 - np.outer(A3 @ u, u) + np.outer(w2, u)
    ...
    def predict_new_state(self, dt, gyro, acc):
        ...
        imu_state = self.state_server.imu_state
        q = imu_state.orientation.copy()
        v = imu_state.velocity.copy()
        p = imu_state.position.copy()
        
        # Compute the dq_dt, dq_dt2 in equation (1) in "MSCKF" paper
        def f(q_in, v_in, p_in):
            R_w_i = to_rotation(q_in)
            dq_dt = 0.5 * omega(gyro) @ q_in
            dv_dt = R_w_i.T @ acc + IMUState.gravity
            dp_dt = v_in
            return dq_dt, dv_dt, dp_dt
        
        # Apply 4th order Runge-Kutta 
        # k1 = f(tn, yn)
        k1_q, k1_v, k1_p = f(q, v, p)
        ...
        imu_state.orientation = q_new / np.linalg.norm(q_new)
        imu_state.velocity = v_new
        imu_state.position = p_new
    ...
    def state_augmentation(self, time):
        ...
        old_P = self.state_server.state_cov
        old_size = old_P.shape[0]
        new_size = old_size + 6
        P = np.zeros((new_size, new_size))

        # Fill in the augmented state covariance.
        P[:old_size, :old_size] = old_P
        P[old_size:new_size, :old_size] = J @ old_P[:21, :]
        P[:old_size, old_size:new_size] = P[old_size:new_size, :old_size].T
        P[old_size:new_size, old_size:new_size] = J @ old_P[:21, :21] @ J.T
        self.state_server.state_cov = P

        # Fix the covariance to be symmetric
        self.state_server.state_cov = 0.5 * (
    ...
    def add_feature_observations(self, feature_msg):
        ...
        curr_feature_num = len(feature_msg.features)
    ...
    def measurement_update(self, H, r):
        ...
        if H.shape[0] == 0 or H.shape[1] == 0 or len(r) == 0:
            return
        ...
        R_cov = self.config.observation_noise * np.identity(H_thin.shape[0])

        # Compute the Kalman gain.
        S = H_thin @ P @ H_thin.T + R_cov
        K = P @ H_thin.T @ np.linalg.inv(S)
        ...
        # Update the IMU state.
        imu_state = self.state_server.imu_state
        delta_theta_imu = delta_x[0:3]
        delta_bg = delta_x[3:6]
        delta_v = delta_x[6:9]
        delta_ba = delta_x[9:12]
        delta_p = delta_x[12:15]
        delta_theta_ext = delta_x[15:18]
        delta_p_ext = delta_x[18:21]

        dq_imu = small_angle_quaternion(delta_theta_imu)
        imu_state.orientation = quaternion_multiplication(dq_imu, imu_state.orientation)
        imu_state.orientation = imu_state.orientation / np.linalg.norm(imu_state.orientation)
        imu_state.gyro_bias += delta_bg
        imu_state.velocity += delta_v
        imu_state.acc_bias += delta_ba
        imu_state.position += delta_p
        dq_ext = small_angle_quaternion(delta_theta_ext)
        imu_state.R_imu_cam0 = to_rotation(dq_ext) @ imu_state.R_imu_cam0
        imu_state.t_cam0_imu += delta_p_ext

        # Update the camera states.
        cam_state_ids = list(self.state_server.cam_states.keys())
        for i, cam_id in enumerate(cam_state_ids):
            cam_state = self.state_server.cam_states[cam_id]
            idx = 21 + i * 6
            delta_theta_cam = delta_x[idx:idx+3]
            delta_p_cam = delta_x[idx+3:idx+6]
            dq_cam = small_angle_quaternion(delta_theta_cam)
            cam_state.orientation = quaternion_multiplication(dq_cam, cam_state.orientation)
            cam_state.orientation = cam_state.orientation / np.linalg.norm(cam_state.orientation)
            cam_state.position += delta_p_cam

        # Update state covariance.
        I_KH = np.identity(P.shape[0]) - K @ H_thin
        self.state_server.state_cov = I_KH @ P @ I_KH.T + K @ R_cov @ K.T
