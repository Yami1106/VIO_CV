...
def generate_trajectory(
    ...
    """
    Generate camera trajectory with a forward-tilted camera.

    look_ahead: how far ahead (in meters) on the ground plane the camera
    looks. This creates a natural tilt angle:
        tilt = arctan(look_ahead / height)
        look_ahead=2.0 at height=6.0 gives ~18° tilt from vertical
        look_ahead=0.0 gives pure downward-facing (old behavior)

    The camera still looks at the floor — just not directly below,
    but slightly ahead in the direction of motion. This gives the
    network visual parallax for altitude (Z) changes.
    """
    dt = 1.0 / rate_hz
    num_samples = int(duration * rate_hz)
