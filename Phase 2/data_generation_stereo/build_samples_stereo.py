"""
build_samples_stereo.py — Build training samples from poses + IMU
------------------------------------------------------------------
Same as build_samples.py but reads/writes to /mnt/data/datasets.

Usage:
    python build_samples_stereo.py --scene_name scene_001_circle
"""
    ...
    with open(csv_path, "r", newline="") as f:
        ...
        for row in reader:
            imu_rows.append({
                ...
                "acc": np.array([float(row["ax"]), float(row["ay"]), float(row["az"])], dtype=float)
            })
    return imu_rows
...
def compute_relative_pose(pose1, pose2):
    p1, p2 = pose1["position"], pose2["position"]
